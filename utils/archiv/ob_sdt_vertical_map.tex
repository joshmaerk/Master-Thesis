...
% ------------------------------------------------------------
% FIGURE 1: Vertikale Theorie-Landkarte (OB -> SDT -> Kompetenz)
% ------------------------------------------------------------
\begin{figure}[ht]
\centering
\begin{tikzpicture}[
...
\node[box, below=of bridges] (sdt) {Self-Determination Theory (SDT)\\\emph{Bedürfnisse $\rightarrow$ Motivation \& Wohlbefinden}};
...
% Bridges details
\node[sub, right=of bridges, xshift=10mm] (bridge_detail) {\textbf{Brücken in Richtung SDT}\\
...
% SDT details
\node[sub, right=of sdt, xshift=10mm] (sdt_detail) {\textbf{SDT-Mechanik (Mikro)}\\
-- Autonomie\\
-- Kompetenz\\
...
% AI as overlay context (connects to bridges/sdt)
...
\end{tikzpicture}
\caption{Vertikale Theorie-Landkarte: Vom Feld Organizational Behavior zur SDT und dem Kompetenzerleben.}
...
% AI playground domains (left column)
...
\node[domainbox, below=of hr] (teams) {\textbf{Human--AI Teams}\\
Koordination, Vertrauen, Rollen\\
\emph{Signaleffekte, Reliance}};
\node[domainbox, below=of teams] (decision) {\textbf{AI in Entscheidungen}\\
Empfehlungen, Prognosen, Analytik\\
\emph{Accountability, Transparency}};

% Focus area (right column): Generative AI as augmented chat-based support
...
% Connections among AI playground
\draw[arrow] (am) -- (hr);
\draw[arrow] (hr) -- (teams);
